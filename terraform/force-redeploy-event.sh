#!/bin/bash
# Force redeploy event service with latest ECR image
...
TASK_FAMILY="event-planner-dev-event-service"

echo "🔄 Force redeploying event service with latest image..."

# 1. Get latest image from ECR
echo "📦 Fetching latest ECR image..."
LATEST_IMAGE=$(aws ecr describe-images \
  --repository-name event-planner/event-service \
...
ECR_IMAGE=$(aws ecr describe-repositories \
  --repository-names event-planner/event-service \
...
echo ""
echo "✅ Event service redeployed successfully!"
