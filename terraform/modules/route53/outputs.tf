...
output "alb_record_name" {
  ...
  value       = var.create_alb_records ? aws_route53_record.alb[0].name : null
}

output "cloudfront_record_name" {
  description = "CloudFront record name"
  value       = var.create_cloudfront_records ? aws_route53_record.cloudfront[0].name : null
}

output "health_check_id" {
  description = "Health check ID"
  value       = var.enable_health_checks && var.create_alb_records ? aws_route53_health_check.alb[0].id : null
}